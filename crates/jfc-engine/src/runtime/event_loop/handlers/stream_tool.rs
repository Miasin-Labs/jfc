...
pub fn dispatch_eager_safe_prefix(state: &mut EngineState, tx: &EventSender) -> Vec<String> {
...
pub fn dispatch_pending_after_stream(state: &mut EngineState, tx: &EventSender) -> bool {
...
pub async fn handle_stream_tool(state: &mut EngineState, tx: &EventSender, tool: Box<ToolCall>) {
...
pub async fn handle_classifier_decision(
    state: &mut EngineState,
    tx: &EventSender,
...
pub fn handle_server_tool_result(
    state: &mut EngineState,
    tx: &EventSender,
...
mod tests {
    ...
    async fn eager_prefix_does_not_dispatch_unsafe_front_regression() {
        ...
        state.pending_tool_calls = vec![bash_tool("b1"), glob_tool("g1")];
    ...
    async fn eager_prefix_drains_only_safe_prefix_before_unsafe_regression() {
        let _guard = StreamingToolExecGuard::enable();
        let mut state = test_app();
        state.pending_tool_calls = vec![glob_tool("g1"), glob_tool("g2"), bash_tool("b1")];
        let (tx, _rx) = tokio::sync::mpsc::channel(8);
        ...
        assert_eq!(state.pending_tool_calls[0].id.as_str(), "b1");
        assert_eq!(state.in_flight_eager_dispatches, 1);
    ...
    async fn eager_prefix_waits_for_auto_mode_classifier_regression() {
        ...
        assert_eq!(state.in_flight_eager_dispatches, 0);
        assert_eq!(state.in_flight_tool_batches, 0);
        assert!(state.pre_dispatched_tool_ids.is_empty());
    }
    ...
    async fn classifier_decision_with_streaming_tool_exec_queues_while_streaming_regression() {
        ...
        assert_eq!(state.pending_classifications, 0);
        assert_eq!(state.pending_tool_calls.len(), 1);
        assert_eq!(state.pending_tool_calls[0].id.as_str(), "g1");
        assert_eq!(state.in_flight_eager_dispatches, 0);
        assert_eq!(state.in_flight_tool_batches, 0);
