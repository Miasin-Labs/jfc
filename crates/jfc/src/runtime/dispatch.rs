...
#[derive(Debug)]
pub(crate) enum FrontendDirective {
...
pub(crate) async fn handle_engine_event(
    ...
    match ev {
        ...
        EngineEvent::Stream(StreamEvent::Tool(tool)) => {
            crate::runtime::event_loop::handlers::stream_tool::handle_stream_tool(state, &tx, tool).await;
        }
        EngineEvent::Tool(ToolEvent::ClassifierDecision {
            ...
            crate::runtime::event_loop::handlers::stream_tool::handle_classifier_decision(state, &tx, tool, blocked, reason,
            )
            .await;
        }
        EngineEvent::Tool(ToolEvent::SetInProgressToolUseIds { action, ids }) => {
            crate::runtime::event_loop::handlers::tools::handle_set_in_progress_tool_use_ids(state, action, ids);
        }
        EngineEvent::Tool(ToolEvent::DeferredToolUse {
            ...
            crate::runtime::event_loop::handlers::tools::handle_deferred_tool_use(state,
                id,
                name,
                input_preview,
                reason,
        ...
        EngineEvent::Tool(ToolEvent::UseSummary {
            ...
            crate::runtime::event_loop::handlers::tools::handle_tool_use_summary(state,
                summary,
                preceding_tool_use_ids,
            );
        }
        EngineEvent::Stream(StreamEvent::ServerToolResult {
            ...
            crate::runtime::event_loop::handlers::stream_tool::handle_server_tool_result(state,
                &tx,
                tool_use_id,
                tool_kind,
                content,
        ...
        EngineEvent::Stream(StreamEvent::Done(stop_reason)) => {
            crate::runtime::event_loop::handlers::stream_done::handle_stream_done(state, &tx, stop_reason).await;
        }

        // ── Stream: error ───────────────────────────────────────
        EngineEvent::Stream(StreamEvent::Error(e)) => {
            crate::runtime::event_loop::handlers::stream_error::handle_stream_error(state, &tx, e).await;
        }
        ...
        EngineEvent::Stream(StreamEvent::FallbackTriggered {
            ...
            crate::runtime::event_loop::handlers::stream_error::handle_fallback_triggered(state,
                &original_model,
                &fallback_model,
                &reason,
            );
        ...
        EngineEvent::Stream(StreamEvent::Usage {
            ...
            crate::runtime::event_loop::handlers::stream_usage::handle_stream_usage(state,
                input_tokens,
                output_tokens,
                cache_read_tokens,
                cache_write_tokens,
        ...
        EngineEvent::Stream(StreamEvent::Lifecycle(status)) => {
            crate::runtime::event_loop::handlers::ui_actions::handle_stream_lifecycle(state, status);
        }
        ...
        EngineEvent::Tool(ToolEvent::Result { tool_id, result }) => {
            crate::runtime::event_loop::handlers::tools::handle_tool_result(state, &tx, tool_id, result);
        ...
        EngineEvent::Compaction(ev) => {
            crate::runtime::event_loop::handlers::compaction::handle_compaction_event(state, &tx, ev).await;
        }
        ...
        EngineEvent::Task(TaskEvent::Started {
            ...
            crate::runtime::event_loop::handlers::task::handle_task_started(state,
                task_id,
                description,
                model_used,
                max_input_tokens,
        ...
        EngineEvent::Task(TaskEvent::Progress {
            ...
            crate::runtime::event_loop::handlers::task::handle_task_progress(state,
                task_id,
                last_tool,
                elapsed_ms,
                tool_use_count,
        ...
        EngineEvent::Task(TaskEvent::Completed {
            ...
            crate::runtime::event_loop::handlers::task::handle_task_completed(state, &tx, task_id, summary, elapsed_ms,
            )
            .await;
        }
        EngineEvent::Task(TaskEvent::Failed { task_id, error }) => {
            crate::runtime::event_loop::handlers::task::handle_task_failed(state, &tx, task_id, error).await;
        }
